      ...
      <div class="content__wrapper">
        ...
        <div class="row">
          ...
          <div class="six columns utility utility--align-right" >
            <router-link to="role-form-page" class="button" ><i class="nc-icon-mini ui-1_circle-add"></i></router-link>
          </div>
        </div>
        ...
        <table class="table table--with-border">
          ...
          <tbody>
            <template v-for="(item, index) in roles.data">
                <tr >
                    <td>{{item.is_sync}}</td>
                    <td>{{item.name}}</td>
                    <td> <span v-for="(item2, index2) in item.permissions">{{item2}}</span></td>
                </tr>
            </template>
          </tbody>
        </table>
