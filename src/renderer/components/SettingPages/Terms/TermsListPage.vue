      ...
      <div class="content__wrapper">
        ...
        <div class="row">
          ...
          <div class="six columns utility utility--align-right" >
            <router-link to="terms-form-page" class="button" ><i class="nc-icon-mini ui-1_circle-add"></i></router-link>
          </div>
        </div>
        ...
        <table class="table table--with-border">
          ...
          <tbody>
            <template v-for="(item, index) in paymentTerms.data">
                <tr >
                    <td>{{item.is_sync}}</td>
                    <td>{{item.name}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      searchVal(msg) {
        this.listPaymentTerms([this.offset, msg.searchText])
      }
    },
    mounted: function () {
      ...
      this.listPaymentTerms([this.offset])
    },
    data () {
      return {
        pagination: {
