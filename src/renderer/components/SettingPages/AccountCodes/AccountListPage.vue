      ...
      <div class="content__wrapper">
        ...
        <div class="row">
          ...
          <div class="six columns utility utility--align-right" >
            <router-link to="account-form-page" class="button" ><i class="nc-icon-mini ui-1_circle-add"></i></router-link>
          </div>
        </div>
        ...
        <table class="table table--with-border">
          <thead>
            <tr>
              <th>SYNC</th>
              <th>Title</th>
              <th>New Account Code</th>
              <th>Old Account Code</th>
            </tr>
          </thead>
          <tbody>
            <template v-for="(item, index) in accountCodes.data">
                <tr >
                    <td>{{item.is_sync}}</td>
                    <td>{{item.name}}</td>
                    <td>{{item.new_account_code}}</td>
                    <td>{{item.old_account_code}}</td>
                </tr>
            </template>
          </tbody>
        </table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      searchVal(msg) {
        this.listAccounts([this.offset, msg.searchText])
      }
    },
    mounted: function () {
