      ...
      <div class="content__wrapper">
        ...
        <div class="row">
          ...
          <div class="six columns utility utility--align-right" >
            <router-link to="chargeability-form-page" class="button" ><i class="nc-icon-mini ui-1_circle-add"></i></router-link>
          </div>
        </div>
        ...
        <table class="table table--with-border">
          ...
          <tbody>
            <template v-for="(item, index) in chargeabilities.data">
                <tr >
                    <td>{{item.is_sync}}</td>
                    <td>{{item.name}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      searchVal(msg) {
        this.listChargeabilitys([this.offset, msg.searchText])
      }
    ...
    mounted: function () {
      ...
      this.listChargeabilitys([this.offset])
    },
    data () {
      return {
        pagination: {
