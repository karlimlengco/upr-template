      ...
      <div class="content__wrapper">
        ...
        <div class="row">
          ...
          <div class="six columns utility utility--align-right" >
            <router-link to="user-form-page" class="button" ><i class="nc-icon-mini ui-1_circle-add"></i></router-link>
          </div>
        </div>
        ...
        <table class="table table--with-border">
          ...
          <tbody>
            <template v-for="(item, index) in users.data">
                <tr >
                    <td>{{item.is_sync}}</td>
                    <td>
                      <router-link :to="{ path: 'user-view-page', query: { userId: item  }}">{{item.username}}</router-link>
                    </td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      searchVal(msg) {
        this.listUsers([this.offset, msg.searchText])
      }
