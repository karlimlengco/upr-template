      ...
      <div class="content__wrapper">
        ...
        <div class="row">
          ...
          <div class="six columns utility utility--align-right" >
            <router-link to="modes-form-page" class="button" ><i class="nc-icon-mini ui-1_circle-add"></i></router-link>
          </div>
        </div>
        ...
        <table class="table table--with-border">
          ...
          <tbody>
            <template v-for="(item, index) in procurementModes.data">
                <tr >
                    <td>{{item.is_sync}}</td>
                    <td>{{item.name}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions([
        'listProcurementModes'
      ]),
      searchVal(msg) {
        this.listProcurementModes([this.offset, msg.searchText])
      }
    },
    mounted: function () {
      ...
      this.listProcurementModes([this.offset])
    },
    data () {
      return {
        pagination: {
