  ...
  markAsSync (units) {
    ...
    db.serialize(function () {
      for (var key in units) {
        var id = units[key].id
        db.run('update chargeability set is_sync=1 where id=?', {
          1: id
        })
      }
    })
  ...
  syncNew (units) {
    db.serialize(function () {
      units.map(function (value, key) {
        ...
        var id = value['id']
        db.get('select id from chargeability where id=?', {1: id}, function (err, row) {
          if (row == null) {
            var fields = ['is_sync']
            var values = ['1']
            var placeholder = "?"
          ...
          if(row != null) {
            if(row.updated_at == null || row.updated_at < value['updated_at']) {
              ...
              db.serialize(function () {
                db.run("UPDATE chargeability SET " + fields.join(',') + " WHERE id = ?", vals, function(err, row) {
                  if (err) {
                    console.log(err)
                  }
                })
